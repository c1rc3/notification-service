...
case class ZConfig(filePath: String) {
  ...
  val config: Config = ConfigFactory.load().withFallback(ConfigFactory.parseFile(new File(filePath)))
  ...
  def print(ps: PrintStream = System.out): Unit = config.entrySet().foreach(x => {
